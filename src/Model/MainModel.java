...
public class MainModel {
    ...
    public ArrayList<CampaignTab.CampaignDataPackage> updateFilters(HashMap<String,String> hashFilters){
        ...
        ArrayList<GraphPoint> CPCOverTime = getDataOverTimePoints("SELECT DATE(date), sum(cost), count(*) from click INNER JOIN person ON click.id = person.id " + whereClause + " group by DATE(date);",true);
        ArrayList<GraphPoint> CPMOverTime = getDataOverTimePoints("SELECT DATE(date), sum(cost)*1000, count(*) from impressions INNER JOIN person ON impressions.id = person.id " + whereClause + " group by DATE(date);",true);
